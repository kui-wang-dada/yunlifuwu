...
{   
    ...
    public function login()
    {
        if (IS_POST) {
            ...
            //}
            $check_posid = M('pos_posinfo')->field('pos_id,pos_name,pos_type,store_id,pos_name,store_name,node_id,func_type,login_token,login_timeout,server_maxposseq')->where(array('pos_id' => $data['pos_id']))->find();
            if (empty($check_posid)) {
                ...
                if ($uptoken !== false) {
                    ...
                    $check_posid['login_timeout']        = strtotime("2050-12-31"); //token过期时间
    ...
    public function getVipUser()
    {
        ...
        $list = M('wp_card_member',null)->where(array('number'=>$cardno))->find();
        if(empty($list)){
            exit(self::returnmsg(ErrorCode::$default_error, '非会员'));
        }
        $res = array(

            'cust_id'=>$list['number'],
            'cust_name'=>$list['username'],
            'cust_tel'=>$list['phone'],
            'cust_jf'=>$list['points'],
            'cust_grade'=>''
            );
...
//当天汇总
        $daysum = M()->query("select sum(verify_amt) sdje,sum(verify_cnt) sdbs,
        ...
        $res = array(
                'sdje'=>round($daysum[0]['sdje']-$daysum[0]['cxje'],2), 
                'sdbs'=>$daysum[0]['sdbs'], 
                'cxje'=>$daysum[0]['cxje'], 
                'cxbs'=>$daysum[0]['cxbs'], 
    ...
    public function querySaleSum()
    {
        ...
        // }
        $daysum = M()->query("select sum(verify_amt) sdje,
        sum(cancel_amt + revoke_amt) cxje,
     IFNULL(sum(CASE `from_type` WHEN '101' then `verify_amt` ELSE 0 end),0) je_101,
     IFNULL(sum(CASE `from_type` WHEN '901' then `verify_amt` ELSE 0 end),0) je_901,
     ...
     IFNULL(sum(CASE `from_type` WHEN '901' then cancel_amt + revoke_amt ELSE 0 end),0) cx_901
        ...
        $monthsum = M()->query("select sum(verify_amt) sdje,
        sum(cancel_amt + revoke_amt) cxje,
     IFNULL(sum(CASE `from_type` WHEN '101' then `verify_amt` ELSE 0 end),0) je_101,
     IFNULL(sum(CASE `from_type` WHEN '901' then `verify_amt` ELSE 0 end),0) je_901,
     ...
     IFNULL(sum(CASE `from_type` WHEN '901' then cancel_amt + revoke_amt ELSE 0 end),0) cx_901
        ...
        $res = array(
            ...
            'thisday'=>array(
                'sdje'=>$daysum[0]['sdje']-$daysum[0]['cxje'],
                'cxje'=>$daysum[0]['cxje'],
                'je_101'=>round(($daysum[0]['je_101']-$daysum[0]['cx_101']),2),
                'je_901'=>round(($daysum[0]['je_901']-$daysum[0]['cx_901']),2),
                'cx_101'=>$daysum[0]['cx_101'],
                'cx_901'=>$daysum[0]['cx_901'],
                ),
            'thismonth'=>array(
                'sdje'=>round(($monthsum[0]['sdje']-$monthsum[0]['cxje']),2),   //保留2位小数
                'cxje'=>$monthsum[0]['cxje'],
                'je_101'=>round(($monthsum[0]['je_101']-$monthsum[0]['cx_101']),2),
                'je_901'=>round(($monthsum[0]['je_901']-$monthsum[0]['cx_901']),2),
    ...
    */
    ...
    public function test2()
    {
        ...
        $key = 'testkey';
        $params      = '{"data":{"pos_id":"1000195407","out_trace_no":"20170105140543522739","trade_no":"2017010521001004020271954439","trade_status":"TRADE_SUCCESS","seller_id":"","seller_mail":"","buyer_id":"2088702862531024","buyer_email":"158****4988","total_amount":0.01,"receipt_amount":0.01,"invoice_amount":0.01,"buyer_pay_amount":0.01,"refund_amount":0,"send_back_amount":0,"coupon_amount":0,"alipayaccount_amount":0,"point_amount":0,"discount_amount":0,"pcard_amount":0,"financeaccount_amount":0,"mcard_amount":0,"mdiscount_amount":0,"mcoupon_amount":0,"pcredit_amount":0,"alipay_fee":0,"receipt_amount_ex_fee":0.01,"gmt_create":"2017-01-05 14:05:45","gmt_payment":"2017-01-05 14:05:45","gmt_refund":"","coupon":"[{\"coupon_id\":\"\",\"coupon_name\":\"\",\"coupon_type\":\"\",\"coupon_amt\":\"\"}]","sign":"C36A062F73F8778C9E1380BF6E8B2D15"}}';
        //$params = json_encode($params);
        $params = json_decode($params,true);
        //$params = json_encode($params);
        //echo($params);
        //exit();
        $url         = 'http://api.skyatlas.cn/index.php/Home/Index/insertZfuData';
    ...
    {   
        ...
        ksort($data);
        \Think\Log::write("Sign urlencode前的请求数据：" . http_build_query($data) . '&key=testkey');
        $pa = $data;
        //有可能有中文，所以要urlencode
        ...
        \Think\Log::write("Sign前的请求数据：" . http_build_query($pa) . '&key=testkey'); //json格式
        \Think\Log::write("后台Sign值：" . md5(http_build_query($pa) . '&key=testkey')); //json格式
        //$params['login_token'] = $token;
        return strtoupper(md5(http_build_query($pa) . '&key=testkey'));
    }
}
/**
 * error code
...
class ErrorCode
{
    ...
    public static $errCode = array('1' => '返回成功', '10001' => 'pos机不存在', '10002' => '账号或密码错误', '10003' => '登录数据不合法', '20001' => '微信刷卡支付失败', '30001' => '京东付款码付款失败', '40001' => '签名效验失败', '40002' => 'token效验失败', '40003' => '不存在的支付方式，请检查','40004' => '终端序列号异常，请重试',  '50001' => '支付超时', '60001' => '数据不合法', '90001' => '系统错误，请联系管理员', '90002' => '其他错误', '90003' => '非法请求', '90004' => '请求超时');
