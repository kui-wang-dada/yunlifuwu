...
class weixin_pay_sdk{
	...
	protected $sub_appid;   //子商户appid
	protected $appsecret;	//微信密钥
	protected $mch_id;		//微信商户ID
	protected $sub_mch_id;  //子商户id
	...
	public function __construct($options = array()){
		//微信参数不存在，就当前目录找wx_config.php配置文件。
		if(empty($options)){
			if(is_file(dirname(__FILE__).'/wx_config.php')){
				$config=include(dirname(__FILE__).'/wx_config.php'); 
			}else{
				exit('wx_config.php配置文件不存在！或者，直接传入微信配置参数。');
			}
		}
	 	$this->appid		    = isset($options['appid'])	? $options['appid'] : $config['appid'];
	 	$this->sub_appid		= isset($options['sub_appid'])	? $options['sub_appid'] : $config['sub_appid'];
		...
		$this->sub_mch_id	    = isset($options['sub_mch_id'])	? $options['sub_mch_id'] : $config['sub_mch_id'];
	...
	*/
	protected static function postXmlCurl($url, $xml, $useCert = false, $second = 30){
		...
		if($useCert == true){
			...
			$path = '/www/web/api_skyatlas_cn/public_html/Public/zhengshu/';
			...
			if(substr($data,0,5)!="<xml>"){
				...
				if(!empty($data)){
					...
					if($data['return_code']=='SUCCESS'){
						...
						self::log($data['return_msg'].json_encode($data));
						return $data;
...
class pay extends weixin_pay_sdk{
	...
	public function unifiedorder($orderid,$body,$total_fee,$notify_url,$atta=array()){
		...
		return $response = self::postXmlCurl(self::UNIFIEDORDER_URL,$xmldata,false,6);
	}
	/*
	* 查询订单
	...
	*/
	public function orderquery($orderid = array()){
		...
		$options=array(
			'appid'	   => $this->appid, //是，公众账号ID
			'sub_appid'	   => $this->sub_appid, //子商户appid，否
			'mch_id'	   => $this->mch_id, //是，商户号
			'sub_mch_id'   => $this->sub_mch_id, //是，子商户商户号
			'nonce_str'=> self::getNonceStr(), //是，随机字符串
		);
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::ORDER_QUERY_URL,$xmldata,false,6);
	}
	/*
	* 关闭订单
	...
	*/
	public function closeorder($orderid){
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::CLOSE_ORDER_URL,$xmldata,false,6);
	}
	/*
	* 申请退款
	* @param $orderid=array(		订单ID（注： transaction_id、out_trade_no二选一 优先微信的订单号transaction_id）
	...
	*/
	public function refund($refund_id,$total_fee,$refund_fee,$orderid,$atta=array()){
		if(!is_array($orderid) || count($orderid) <= 0){
			if(self::$IS_DEBUG){
    				exit("申请退款: $orderid参数请传入数组。如：array('out_trade_no'=>'20150806125346')");
			}else{
				self::log("申请退款: $orderid参数请传入数组。如：array('out_trade_no'=>'20150806125346')");
			}
    		}
		$options=array(
			'appid'	   => $this->appid, //是，公众账号ID
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::REFUND_URL,$xmldata,true,6);
	}
	/*
	* 查询退款
	...
	*/
	public function refundquery($orderid,$atta=array()){
		if(!is_array($orderid) || count($orderid) <= 0){
			if(self::$IS_DEBUG){
    				exit("查询退款: $orderid参数请传入数组。如：array('out_trade_no'=>'20150806125346')");
			}else{
				self::log("查询退款: $orderid参数请传入数组。如：array('out_trade_no'=>'20150806125346')");
			}
    		}
		$options=array(
			'appid'	   => $this->appid, //是，公众账号ID
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::REFUND_QUERY_URL,$xmldata,false,6);
	}
	/*
	* 下载对账单
	...
	*/
	public function downloadbill($date,$atta=array(),$is_download_excel=false){
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		$response = self::postXmlCurl(self::DOWNLOAD_BILL_URL,$xmldata,false,6);
		$data = explode("\n",$response);
	...
	public function shorturl($url){
		...
		$xmldata=self::ArrayXml($options);
		$response = self::postXmlCurl(self::SHORT_URL,$xmldata,false,6);
	}
}
...
class weixin_pay_card extends pay{
	...
	* @return array
	*/
	public function micropay($data=array(),$atta=array()){
		$options=array(
			'appid'	   => $this->appid, //是，公众账号ID
			'sub_appid'	   => $this->sub_appid, //子商户appid，否
			'mch_id'	   => $this->mch_id, //是，商户号
			...
			'body'	=> $data['body'], //是, 商品或支付单简要描述
			'detail'	=> (!empty($atta['detail'])) ? $atta['detail'] : '',	//否, 商品详细列表Json格式
			'attach'=> (!empty($atta['attach'])) ? $atta['attach'] : '',	//否, 附加数据
			'out_trade_no'=> $data['orderid'], //是，商户系统内部的订单号,32个字符内。
			'total_fee'	  => $data['total_fee'], //是,订单总金额,把元转换为分。
			...
			'auth_code'	  => $data['auth_code'],  //是，扫码支付授权码，设备读取用户微信中的条码或者二维码信息
		); 
		foreach($options as $opk=>$opv){ if(empty($opv)){ unset($options[$opk]); } }	//删除为空的参数
		$sign=$this->get_sign($options);		//获取签名
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::MICROPAY_URL,$xmldata,false,6);	
	}
	/*
	* 撤销订单
	* @param $orderid=array(		订单ID（注： transaction_id、out_trade_no二选一 优先微信的订单号transaction_id）
	...
	*/
	public function reverse($orderid){
		if(!is_array($orderid) || count($orderid) <= 0){
			if(self::$IS_DEBUG){
    				exit("撤销订单: $orderid参数请传入数组。如：array('out_trade_no'=>'20150806125346')");
			}else{
				self::log("撤销订单: $orderid参数请传入数组。如：array('out_trade_no'=>'20150806125346')");
			}
    		}
		$options=array(
			'appid'	   => $this->appid, //是，公众账号ID
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::REVERSE_URL,$xmldata,true,6);
	}
	/*
	* 授权码查询OPENID接口
	...
	*/
	public function authcodetoopenid($auth_code){
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::AUTHCODE_OPENID_URL,$xmldata,false,6);
	}
}
//微信公众号支付
...
class weixin_coupon extends weixin_pay_sdk{
	...
	*/
	public function send_coupon($openid,$stock_id,$partnerid){
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::SEND_COUPON_URL,$xmldata,true,6);
	}
	/*
	* 查询代金券批次信息
	* @param String(32)		$stock_id  代金劵对应的批次号
	...
	*/
	public function query_coupon_stock($stock_id,$atta=array()){
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::QUERY_COUPON_STOCK_URL,$xmldata,false,6);
	}
	/*
	* 查询代金券信息
	* @param String(32)		$stock_id  代金劵对应的批次号
	...
	*/
	public function querycouponsinfo($stock_id,$openid,$coupon_id,$atta=array()){
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::QUERY_COUPON_INFO_URL,$xmldata,false,6);
	}
}
//微信红包
class weixin_bonus extends weixin_pay_sdk{
	...
	*/
	public function sendredpack($openid,$orderid,$money,$bonus_data=array()){
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::SEND_RED_PACK,$xmldata,true,6);
	}
	/*
	* 发送裂变现金红包
	...
	*/
	public function sendgroupredpack($openid,$orderid,$money,$bonus_data=array()){
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::SEND_GROUP_RED_PACK,$xmldata,true,6);
	}
	/*
	* 查询红包的具体信息
	...
	*/
	public function gethbinfo($orderid,$atta=array()){
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::QUERY_RED_PACK_INFO,$xmldata,true,6);
	}
}
//微信付款
class weixin_payment extends weixin_pay_sdk{
	...
	*/
	public function transfers($openid,$orderid,$amount,$desc='提现',$atta=array()){
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::TRANSFERS_URL,$xmldata,true,6);
	}
	/*
	* 查询企业付款
	...
	*/
	public function gettransferinfo($orderid){
		...
		$options['sign']=$sign;
		$xmldata=self::ArrayXml($options);
		return $response = self::postXmlCurl(self::QUERY_TRANSFERS_URL,$xmldata,true,6);
	}
}
//微信支付结果通知类
...
class jd_pay_sdk{
	 ...
	 * 参数按字母大小排序
	 */
	protected function _sortParams($data) {
	...
	protected function _createSign($text, $salt) {
		$md5 = md5 ( $text . "" . $salt );
		return $md5;
	}
	
	protected function _httpRequest($url, $param = '') {
		$ch = curl_init ();
		curl_setopt ( $ch, CURLOPT_URL, $url );
...
//京东付款码支付
class jd_paycode extends jd_pay_sdk{
  
  /**
	 * @var 获取订单状态接口地址
	 */
	const API_URL = 'https://pcplatform.jdpay.com/api/pay';
	/**
	 * @var 商户md5-key
	 */
	const SALT = "SMF360080003233950018";
	 ...
	 */
	public function execute(array $args,$paykey='') {
		$this->_params ['order_no'] = $args ['order_no'];
		$this->_params ['merchant_no'] = $args ['merchant_no'];
		$this->_params ['amount'] = $args ['amount'];
		$this->_params ['seed'] = $args ['seed'];
		$this->_params ['notify_url'] = $args ['notify_url'];
		$this->_params = $this->_sortParams($this->_params);   //排序
		$text = $this->_buildQuery ($this->_params);           //拼接
		$this->_params ['sign'] = $this->_createSign ( $text, $paykey ); //生成签名
		$query = $this->_buildQuery ($this->_params);          //组装post数据
		$content = $this->_httpRequest ( self::API_URL, $query );
		if (empty ( $content )) {
			exit ();
		}
		...
		if ($result ['is_success'] == 'Y') {
			...
			$errorCode = $result ['fail_code'];
			$errorDes = $result ['fail_reason'];
			//return "code:" . $errorCode . ",des:" . $errorDes ;
			return $result;
		}
	}
}